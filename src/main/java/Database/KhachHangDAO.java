...
public class KhachHangDAO implements DAO<KhachHang> {
	...
	public ArrayList<KhachHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				KhachHang kh = new KhachHang(maKhacHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, diaChi, diaChiNhanHang, diaChiMuaHang, ngaySinh, soDienThoai, email, dangKyNhanBangTin, email, ngaySinh, dangKyNhanBangTin, duongDanAnh);
				ketQua.add(kh);
			}
	...
	public KhachHang selectById(KhachHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				String soDienThoai = rs.getString("sodienthoai");
				String email = rs.getString("email");
				boolean dangKyNhanBangTin = rs.getBoolean("dangkinhanbantin");
	...
	public int insert(KhachHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "INSERT INTO khachhang (makhachhang, tendangnhap, matkhau, hoten, gioitinh, diachi, diachinhanhang, diachimuahang, ngaysinh, sodienthoai, email, dangkinhanbantin) "
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public int update(KhachHang t) {
		...
		try {
			...
			String sql = "UPDATE khachhang " + " SET " + " tendangnhap=?" + ", matkhau=?" + ", hoten=?" + ", gioitinh=?"
					...
					+ ", email=?" + ", dangkinhanbantin=?" + " WHERE makhachhang=?";
	...
	public KhachHang selectByUsernameAndPassword(KhachHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				ketQua = new KhachHang(maKhacHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, diaChi, diaChiNhanHang, diaChiMuaHang, ngaySinh, soDienThoai, email, dangKyNhanBangTin, maXacThuc, thoiGianHieuLucCuaMaXacThuc, trangThaiXacThuc);
			}
